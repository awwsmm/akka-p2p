...
object Actions extends StrictLogging {
  ...
  def connect(address: Address)(implicit system: ActorSystem[User.Command]): Option[String] = {
    val msg = s"Attempting to request connect to peer at $address"
    logger.debug(msg)
    system.ref ! User.RequestConnection(address, x => println(s"R: $x"), 10.seconds)
    Some(msg)
  }
